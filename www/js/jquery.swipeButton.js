		...
		return this.filter('[data-swipeurl]').each(function(i, el){
			...
			$e.on(o.direction, function ( e ) {
				...
				if (!cnt) {
					...
					$li.parent('ul').one('click tap', function(e){					
						e.stopPropagation();
						e.preventDefault();
						$('div.ui-btn, .' + o.btnClass, $(this)).animate({ width: 'toggle' }, 200, function(e) {
							$(this).remove();						
						});
						$(this).parent('ul').off('click tap');
					});	
				...
				/*	$li.parent('ul').off('click tap', function(e){
						$('div.ui-btn, .' + o.btnClass, $(this)).animate({ width: 'toggle' }, 200, function(e) {
						$(this).remove();
						});
					});*/
